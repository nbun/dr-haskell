...
getConverageOutput m = do
    ...
    mixtix <- withSystemTempDirectory "drhaskell-lint." -- compile, run and check inside a temp dir
                    $ runInTempDir plainFile
    case mixtix of
        Just (mix, tix) -> return $ parseMT mix tix -- parse the check output
        Nothing         -> return []

-- | Takes the Mix and the Tix information provided by the hpc impl and converts it into coverage information
...
parseMT mix@(Mix _ _ hash1 _ mixpos) (Tix (TixModule _ hash2 _ tixpos:xs))
    | hash1 == hash2 = -- get the right mix tix combination
        let zipped = zip tixpos mixpos
            ...
            filteredMixPos = foldr (\(_,x) xs -> x : xs) [] filtered -- filter only the lines hit
    ...
    | otherwise = parseMT mix (Tix xs) -- next tix if hashed dont match
        where loopMixPos :: [MixEntry] -> [Lint] -- build the lintoutput
              ...
              restorePosition (sl, sc, el, ec) = (sl - 1, sc, el - 1, ec) -- some position adjustment. maybe this needs some adjustments if there are more includes at the start of the file
...
runInTempDir plainFile tmpDir = do
    ...
    datadir <- getDataDir -- get datadir from cabal config for include search inside of ghc
    writeFile (tmpDir ++ "/tmp.hs") plainFile -- write the actual file which we want to analyse
    tmpExists <- doesFileExist (tmpDir ++ "/tmp.hs")
    if tmpExists
    then do (_, _, _) <- readProcessWithExitCode "ghc" ["-fhpc", "tmp.hs", "-o", "tmp", "-idirs:" ++ datadir ++ "/TargetModules"] [] -- run ghc with hpc flag and append include dir from cabal datadir
            tmpCompExists <- doesFileExist (tmpDir ++ "/tmp")
            if tmpCompExists
            then do (_, _, _) <- readProcessWithExitCode "./tmp" [] [] -- run the compiled file to generate coverage output
                    hpcDirExists <- doesDirectoryExist (tmpDir ++ "/.hpc")
                    tixExists <- doesFileExist (tmpDir ++ "/tmp.tix")
                    if hpcDirExists && tixExists -- are all files we need present
                    then do mix <- readMix [tmpDir ++ "/.hpc"] (Left "Main")
                            tix <- readTix (tmpDir ++ "/tmp.tix")
                            case tix of
                                Just t  -> return $ Just (mix, t) -- return mix and tix
